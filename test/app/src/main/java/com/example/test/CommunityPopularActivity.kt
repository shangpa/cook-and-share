...
class CommunityPopularActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_community_popular)
        ...
        dropDown.setOnClickListener {
            ...
            popup.setOnMenuItemClickListener { item: MenuItem ->
                when (item.title) {
                    "인기 게시판" -> {
                        // 현재 화면이 인기 게시판이면 아무것도 안 해도 됨
                        popularPost.text = item.title
                    }
                    "요리 게시판" -> {
